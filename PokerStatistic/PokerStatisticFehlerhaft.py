...
def straight(cards,statistic):
    ...
    for x in cards[1:len(cards)]:
        if(x-cards[pos] == 1 or x-cards[0] == 13):
            pos += 1
...
def straightFlush(colors,cards,statistic):
    ...
    cards.sort()
    for x in cards[1:len(cards)]:
        if ((x - cards[pos] == 1 or x - cards[0] == 13) and colors.count(x) == 5):
            pos += 1
        else:
            return statistic
...
def royalFlush(numbers,colors,statistic):
    numbers.sort()
    for x in colors:
        if (colors.count(x) == 5 and numbers == [9,10,11,12,13]):
            return statistic.append(10)
        else:
            return statistic

if __name__ == '__main__':
    symbols=["Highest Card","Pair","Double Pair","Triblet","Straight","Flush","Full House","Quad","Straight Flush","Royal Flush"]
    statistic = []
    oldstatistic = []
    stat=[]
    ziehungen = 100000
